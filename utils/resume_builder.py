...
class ResumeBuilder:
    ...
    def _add_profile_image(self, table_cell, image_data):
        """Helper function to add profile image to a table cell"""
        try:
            if image_data and isinstance(image_data, dict) and 'data' in image_data:
                import base64
                from io import BytesIO
                ...
                # Add image to cell (1.5 inches width)
    ...
    def build_modern_template(self, doc, data):
        ...
        try:
            ...
            # Create header table for name and image layout
            header_table = doc.add_table(rows=1, cols=2)
            header_table.autofit = False
            header_table.allow_autofit = False
            
            # Set column widths
            header_table.columns[0].width = Inches(1.5)  # Left column for image
            header_table.columns[1].width = Inches(4.5)  # Right column for text
            
            # Left cell - Profile image
            left_cell = header_table.rows[0].cells[0]
            if data.get('profile_image'):
                self._add_profile_image(left_cell, data['profile_image'])
            
            # Right cell - Name and contact info
            right_cell = header_table.rows[0].cells[1]
            
            # Add name
            name_paragraph = right_cell.paragraphs[0]
            name_run = name_paragraph.add_run(data['personal_info']['full_name'].upper())
            name_run.font.size = Pt(24)
            name_run.font.bold = True
            name_run.font.color.rgb = RGBColor(41, 128, 185)
            name_run.font.name = 'Arial'
            name_paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
            
            # Add role/title if available
            if data['personal_info'].get('title'):
                title_para = right_cell.add_paragraph(data['personal_info']['title'])
                title_para.style = contact_style
            
            # Contact information
            contact_info = right_cell.add_paragraph()
            ...
            if data['personal_info'].get('linkedin') or data['personal_info'].get('portfolio'):
                links = right_cell.add_paragraph()
    ...
    def build_professional_template(self, doc, data):
        ...
        try:
            ...
            # Add name
            name_paragraph = right_cell.paragraphs[0]
            name_run = name_paragraph.add_run(data['personal_info']['full_name'])
            name_run.font.size = Pt(24)
            name_run.font.bold = True
            ...
            if contact_parts:
                contact = right_cell.add_paragraph()
    ...
    def build_minimal_template(self, doc, data):
        ...
        try:
            ...
            # Add name
            name = right_cell.paragraphs[0]
            name_run = name.add_run(personal['full_name'])
            name_run.font.size = Pt(28)
            name_run.font.bold = True
            name_run.font.color.rgb = RGBColor(33, 33, 33)
    ...
    def build_creative_template(self, doc, data):
        ...
        try:
            ...
            # Add name with emojis
            name_paragraph = right_cell.paragraphs[0]
            name_run = name_paragraph.add_run('✨ ' + data['personal_info']['full_name'] + ' ✨')
            name_run.font.size = Pt(24)
            name_run.font.bold = True
            name_run.font.color.rgb = RGBColor(155, 89, 182)
            name_run.font.name = 'Arial'
            name_paragraph.alignment = WD_ALIGN_PARAGRAPH.CENTER
            
            # Add role/title if available
            if data['personal_info'].get('title'):
                title = right_cell.add_paragraph('💫 ' + data['personal_info']['title'])
